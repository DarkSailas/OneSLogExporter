...
# Clean stale binary artifacts to ensure fresh win-x64 real implementations overwrite old stubs
Get-ChildItem -Path $publishServiceDir -Include *.dll,*.exe,*.pdb,*.deps.json,*.runtimeconfig.json -File -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force
Get-ChildItem -Path $publishGuiDir -Include *.dll,*.exe,*.pdb,*.deps.json,*.runtimeconfig.json -File -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force
...
foreach ($target in $syncTargets) {
    ...
    Get-ChildItem -Path (Join-Path $tPublish "Gui") -Include *.dll,*.exe,*.pdb,*.deps.json,*.runtimeconfig.json -File -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force
    Get-ChildItem -Path (Join-Path $tPublish "Service") -Include *.dll,*.exe,*.pdb,*.deps.json,*.runtimeconfig.json -File -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force
