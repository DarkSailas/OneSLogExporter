...
public sealed class FileDumper
{
    ...
    private (string FilePath, FileMode Mode) GetTargetFile(string targetDir, string pattern, string prefix, bool forceNewSlot = false)
    {
        ...
        if (basePattern.Contains("{N}", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (oldestFile != null)
            {
                // ВАЖНО: НИКОГДА не вызываем file.Delete() перед FileMode.Create!
                // В Windows вызов Delete() переводит файл в состояние DeletePending,
                // из-за чего последующий new FileStream(..., FileMode.Create) выбрасывает
                // System.UnauthorizedAccessException: Access to the path ... is denied!
                // FileMode.Create усекает существующий файл на месте без вызова Delete().
