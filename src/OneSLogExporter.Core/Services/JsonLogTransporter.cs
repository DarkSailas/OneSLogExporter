    ...
    public async ValueTask TransportEventLogsAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (var fileInfo in files)
            {
                ...
                // Если размер файла меньше зафиксированного смещения, файл был перезаписан по кольцу слотов дампа.
                // Сбрасываем смещение чтения в 0, чтобы гарантированно прочитать все новые записи.
                if (fileInfo.Length < lastPos)
                {
                    _logger.LogInformation("Файл дампа ЖР {FileName} был перезаписан/ротирован (размер {Length} < {LastPos}). Сброс позиции на начало файла.",
                ...
                while ((line = await reader.ReadLineAsync(ct).ConfigureAwait(false)) != null)
                {
                    ...
                    if (batch.Count >= batchSize)
                    {
                        ...
                        _stateTracker.MarkFilePosition(file, reader.CurrentLineStartOffset, fileInfo.Length);
                ...
                {
                    _stateTracker.MarkFilePosition(file, fs.Length, fileInfo.Length);
                    await _stateTracker.SaveAsync(ct).ConfigureAwait(false);
                }
    ...
    public async ValueTask TransportTechLogsAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (var fileInfo in files)
            {
                ...
                if (fileInfo.Length < lastPos)
                {
                    _logger.LogInformation("Файл дампа ТЖ {FileName} был перезаписан/ротирован (размер {Length} < {LastPos}). Сброс позиции на начало файла.",
                        fileInfo.Name, fileInfo.Length, lastPos);
                    lastPos = 0;
                }
                else if (fileInfo.Length == lastPos)
