...
public sealed class SessionCacheService : IAsyncDisposable, IDisposable
{
    ...
    {
        // Всегда используем локальный высокоскоростной NVMe/SSD каталог (%TEMP%\OneSLogExporter\temp)
        // Исключаем сетевые и облачные диски (Google Drive G:\, OneDrive и др.) с высокой задержкой sync
        var localTemp = Path.Combine(Path.GetTempPath(), "OneSLogExporter", "temp");
        try
        {
        ...
        {
            var fallback = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "temp");
    ...
    /// <summary>
    /// Однократное фоновое построение вторичных индексов после завершения массовой загрузки логов.
    /// </summary>
