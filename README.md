# 1C: Log Exporter (OneSLogExporter) v1.9.0
...
## Особенности работы с памятью и оптимизация

При обработке сотен миллионов записей критически важна стабильность памяти процесса:
1. **Workstation GC**: в службе настроен Workstation Garbage Collection (`ServerGarbageCollection = false`, `RetainVM = false`). В отличие от Server GC, который на многоядерных машинах может удерживать десятки гигабайт памяти впрок, Workstation GC оперативно возвращает освобожденные страницы операционной системе (`VirtualFree`), удерживая рабочий набор службы в разумных пределах (обычно 100–300 МБ).
2. **Исключение LOH-аллокаций**: размер промежуточных сериализационных буферов ограничен 64 КБ, предотвращая засорение кучи больших объектов (Large Object Heap) и фрагментацию виртуального адресного пространства.
3. **Автонарезка файлов дампа (Intra-batch Rolling)**: если за один цикл опроса 1С сгенерировала огромный объем записей (например, 300–400 тысяч транзакций), дампер на лету делит данные на аккуратные порции по ~16 МБ, плавно заполняя кольцевые слоты `data_evlog_{N}.json` и не выбиваясь из общего лимита `MaxTotalSizeMb`.
4. **Ограниченный сессионный кэш**: динамический маппинг сессий на пользователей 1С ограничен емкостью 50 000 элементов, что исключает утечки памяти при круглосуточной работе службы.

---

## Полезные запросы для проверки ClickHouse

После запуска службы можно проверить приход данных простым запросом:

```sql
-- Проверка общего количества записей и диапазона дат
SELECT count(), min(Date), max(Date) FROM default.eventlog;

-- Топ пользователей по числу событий за последние сутки
SELECT User, AppTypeName, count() 
FROM default.eventlog 
WHERE Date > now() - INTERVAL 1 DAY 
GROUP BY User, AppTypeName 
ORDER BY count() DESC 
LIMIT 15;

-- Проверка корректности разрешения словарей (не должно быть значений 'User #...')
SELECT 
    count() AS Total,
    countIf(startsWith(User, 'User #')) AS UnresolvedUsers,
    countIf(User = '') AS EmptyUsers
FROM default.eventlog 
